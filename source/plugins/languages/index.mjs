  ...
  try {
    ...
    if ((sections.includes("recently-used")) && (imports.metadata.plugins.languages.extras("indepth", {extras}))) {
      try {
        ...
        languages["stats.recent"] = await recent_analyzer({login, data, imports, rest, context, account}, {skipped, pathsIgnored:paths_ignored, categories:_recent_categories ?? categories, days:_recent_days, load:_recent_load, timeout})
    ...
    if ((indepth) && (imports.metadata.plugins.languages.extras("indepth", {extras}))) {
      try {
        ...
        Object.assign(languages, await indepth_analyzer({login, data, imports, rest, context, repositories:repositories.concat(_indepth_custom)}, {skipped, pathsIgnored:paths_ignored, categories, timeout}))
